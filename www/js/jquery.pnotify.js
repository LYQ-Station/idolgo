...
(function($) {
	...
	$.extend({
		...
		pnotify: function(options) {
			...
			if (typeof opts.pnotify_max_height == "string")
			{
				pnotify.text_container.css("max-height", opts.pnotify_max_height);
				pnotify.text_container.css("overflow", 'auto');
			}
	...
	var dom_event = function(e, orig_e){
		...
		if (document.createEvent && this.dispatchEvent) {
			...
			if (e.match(re_mouse_events)) {
				...
				$(this).offset();
				event_object = jwindow.document.createEvent("MouseEvents");
				...
				event_object = jwindow.document.createEvent("UIEvents");
				event_object.initUIEvent(e, orig_e.bubbles, orig_e.cancelable, orig_e.view, orig_e.detail);
			} else if (e.match(re_html_events)) {
				event_object = jwindow.document.createEvent("HTMLEvents");
			...
			event_object = jwindow.document.createEventObject(orig_e);
...
top['notify'] = function (title, text, options)
{
	options = options || {};
	
	options.pnotify_title = title;
	options.pnotify_text = text;
	$.pnotify(options);
}

top['msger'] = function (title, text, options)
{
	options = options || {};
	...
	options.pnotify_addclass = "msger";
	options.pnotify_stack = stack_bottomright;
	options.pnotify_width =  options.pnotify_width ? options.pnotify_width : '350px';
	options.pnotify_max_height = options.pnotify_max_height ? options.pnotify_max_height : '100px';
